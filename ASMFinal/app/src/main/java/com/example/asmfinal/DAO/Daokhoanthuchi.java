...
public class Daokhoanthuchi {
    ...
    public ArrayList<KhoanThuChi> readAll(){
        ...
        Cursor cs = db.rawQuery("Select khoan.MATC,khoan.TENKHOANTC,khoan.NGAY,khoan.TIEN,khoan.GhiChu,khoan.MALOAI,loai.TENLOAI,loai.TRANGTHAI" +
                " From KHOAN_TC khoan inner join LOAI_TC loai on(khoan.MALOAI=loai.MALOAI) AND loai.TRANGTHAI like'%Thu%'",null);
        cs.moveToFirst();
    ...
    public ArrayList<KhoanThuChi> gettheongay(String ngaydau, String ngaycuoi){
        ArrayList<KhoanThuChi> datakhoanthu = new ArrayList<>();
        SQLiteDatabase db = tc.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT KHOAN_TC.tenkhoantc,KHOAN_TC.tien FROM KHOAN_TC,LOAI_TC" +
                " WHERE KHOAN_TC.maloai=LOAI_TC.maloai AND KHOAN_TC.NGAY BETWEEN '"+ngaydau+"'AND'"+ngaycuoi+"' AND LOAI_TC.TRANGTHAI LIKE 'Thu';",null);
        cursor.moveToFirst();
        while ((!cursor.isAfterLast())){
            String tentc =cursor.getString(0);
            double tien =cursor.getDouble(1);
    ...
    public double gettongtien(String ngaydau, String ngaycuoi){
